...
This is an **alpha** (`0.50.0-alpha`). Expect bugs, rough edges, and features that are still placeholders (the Kirikiri engine runner, for one, is a stub — not yet functional). Debug-signed builds for now; a proper release-signed pipeline comes later.
